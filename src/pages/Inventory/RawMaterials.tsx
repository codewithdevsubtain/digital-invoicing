...
import { Plus, Search, X, Pencil, Power, PowerOff, PackageOpen, History, Calendar } from 'lucide-react'
...
const emptyForm = { name: '', category_id: '' as string | number, unit_id: '' as string | number, reorder_level: '', standard_cost: '', hsn_code: '', description: '', item_type: 'raw_material' as const }
...
export default function RawMaterials() {
  ...
  const loadReference = useCallback(async () => {
    try {
      ...
      setUnits(u)
      setCategories(c.filter((cat) => !cat.item_type || cat.item_type === 'raw_material'))
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const list = await api.inventory.listItems(user.id, { item_type: 'raw_material', search: search || undefined })
      setItems(list)
    } catch (err) {
      addToast({ type: 'error', title: 'Error', message: 'Failed to load raw materials' })
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editing) {
        ...
        addToast({ type: 'success', title: 'Updated', message: 'Raw material updated' })
      } else {
        await api.inventory.createItem(user.id, data)
        addToast({ type: 'success', title: 'Created', message: 'Raw material created' })
      }
  ...
  const handleAdjust = async () => {
    ...
    try {
      ...
      addToast({ type: 'success', title: 'Adjusted', message: 'Stock adjusted successfully' })
      setShowAdjust(false)
  ...
  return (
    <div>
      <PageHeader title="Raw Materials" subtitle="Manage raw material inventory">
        <button onClick={openAdd} className="btn-primary gap-2"><Plus size={18} /> Add Raw Material</button>
      </PageHeader>
      ...
      <div className="mt-6">
        ...
        <DataTable
          ...
          columns={[
            ...
            { key: 'reorder_level', header: 'Reorder', render: (r) => r.reorder_level > 0 ? r.reorder_level : '-' },
            { key: 'standard_cost', header: 'Avg Cost', render: (r) => formatCurrency(r.standard_cost) },
      ...
      {/* Add/Edit Modal */}
      <FormModal open={showModal} title={editing ? 'Edit Raw Material' : 'Add Raw Material'} onClose={() => setShowModal(false)} onSubmit={handleSubmit}>
        <div>
          <label className="label-text mb-1">Name *</label>
          <input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="input-field" placeholder="e.g., GI Sheet 22G" />
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div>
            <label className="label-text mb-1">Standard Cost (PKR)</label>
        ...
        <div>
          ...
          <input value={form.hsn_code} onChange={(e) => setForm({ ...form, hsn_code: e.target.value })} className="input-field" placeholder="e.g., 7219.30" />
        </div>
        <div>
            ...
            {detailLoading ? <Loading text="Loading details..." /> : (
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm">
                  ...
                  <div><span className="text-gray-500">Standard Cost:</span> <span className="font-medium">{formatCurrency(detail?.standard_cost ?? 0)}</span></div>
                ...
                <FormModal open={showAdjust} title="Adjust Stock" onClose={() => setShowAdjust(false)} onSubmit={handleAdjust} submitLabel="Adjust">
                  ...
                  <div>
                    ...
                    <textarea value={adjForm.reason} onChange={(e) => setAdjForm({ ...adjForm, reason: e.target.value })} className="input-field" rows={2} placeholder="e.g., Physical count adjustment, damaged goods" />
                  </div>
                </FormModal>
